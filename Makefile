...
build:
	docker build . -t $(CTN1) -f Dockerfile-pandoc
	docker build . -t $(CTN2) -f Dockerfile-printout
...
test_file:
	docker run --rm --volume "$(shell pwd)/${TESTDIR}:/data" $(CTN1) $(CC) -f markdown --self-contained ${MDDIR}/01_Doc_en.md -c ${CSSDIR}/markdown.css -c ${CSSDIR}/markdown-pdf.css -o ${HTMLDIR}/01_Doc_en.html 
	docker run --rm $(TZ) --volume "$(shell pwd)/${TESTDIR}:/data" $(CTN2) "${HTMLDIR}/01_Doc_en.html ${PDFDIR}/01_Doc_en.pdf"
	docker run --rm --volume "$(shell pwd)/${TESTDIR}:/data" $(CTN1) $(CC) -f markdown --self-contained ${MDDIR}/01_Doc_ja.md -c ${CSSDIR}/markdown.css -c ${CSSDIR}/markdown-pdf.css -o ${HTMLDIR}/01_Doc_ja.html
